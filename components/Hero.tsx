...
const LiveChat: React.FC = () => {
  ...
  const send = async () => {
    ...
    try {
      ...
      const reply = data?.content?.[0]?.text || 'Sorry, I couldn\'t get a response. Please email us at info@ia7globaltech.eu';
    ...
    <div style={{ border: '1px solid #e8eeff', borderRadius: 20, overflow: 'hidden', background: '#fff', boxShadow: '0 12px 48px rgba(0,85,255,0.1)', display: 'flex', flexDirection: 'column', height: 520 }}>
      ...
      <div style={{ background: '#0055ff', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: 'auto' }}>
          <a href="https://jgai-demo.netlify.app/" target="_blank" rel="noopener noreferrer"
            style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)', background: 'rgba(255,255,255,0.15)', padding: '4px 10px', borderRadius: 100, textDecoration: 'none' }}>
            Full demo →
          </a>
        </div>
      </div>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.from === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{
              ...
              background: m.from === 'user' ? '#0055ff' : '#fff',
              color: m.from === 'user' ? '#fff' : '#0f0f0f',
              borderRadius: m.from === 'user' ? '14px 14px 2px 14px' : '14px 14px 14px 2px',
              border: m.from === 'user' ? 'none' : '1px solid #e8eeff',
              whiteSpace: 'pre-wrap',
            }}>
              {m.text}
            </div>
        ...
        {loading && (
          <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <div style={{ padding: '10px 14px', background: '#fff', border: '1px solid #e8eeff', borderRadius: '14px 14px 14px 2px', display: 'flex', gap: 4, alignItems: 'center' }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#0055ff', opacity: 0.4, animation: `bounce 1.2s ${i * 0.2}s infinite` }} />
              ))}
            </div>
          </div>
        )}
      ...
      {messages.length === 1 && (
        <div style={{ padding: '8px 16px', background: '#fafbff', display: 'flex', gap: 6, flexWrap: 'wrap', borderTop: '1px solid #e8eeff' }}>
          {['What services do you offer?', 'How fast can you build a chatbot?', 'Do you work in German?'].map(q => (
            <button key={q} onClick={() => { setInput(q); }} style={{ fontSize: 11, color: '#0055ff', background: '#eef3ff', border: '1px solid #e0eaff', borderRadius: 100, padding: '5px 11px', cursor: 'pointer' }}>
              {q}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding: '12px 14px', borderTop: '1px solid #e8eeff', background: '#fff', display: 'flex', gap: 8, alignItems: 'flex-end', flexShrink: 0 }}>
        <textarea
          ...
          onKeyDown={onKey}
          placeholder="Ask me anything about IA7..."
          rows={1}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center', minHeight: 'calc(100vh - 80px)', paddingBottom: 64 }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ position: 'absolute', top: -14, left: 20, zIndex: 10, background: '#fff', border: '1px solid #e8eeff', borderRadius: 8, padding: '7px 14px', display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 16px rgba(0,85,255,0.08)' }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 500, color: '#0f0f0f' }}>Live — Talk to our AI</span>
          </div>
          <LiveChat />
          <div style={{ position: 'absolute', bottom: -14, right: 20, zIndex: 10, background: '#fff', border: '1px solid #e8eeff', borderRadius: 8, padding: '7px 14px', boxShadow: '0 4px 16px rgba(0,85,255,0.08)' }}>
            <div style={{ fontSize: 11, color: '#888', marginBottom: 1 }}>Want the full version?</div>
            <a href="https://jgai-demo.netlify.app/" target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, fontWeight: 500, color: '#0055ff' }}>jgai-demo.netlify.app →</a>
          </div>
        </div>
      </div>
    </div>
